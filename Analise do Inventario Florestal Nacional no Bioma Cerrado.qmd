...
# Exportar para a Deobra - Todas as espécies citadas 
# produtos_mais_citados %>%
#   st_drop_geometry() %>%
#   select(-Quantidade) %>%
#   export("produtos_mais_citados_tabela_sociologica.xlsx")
...
# Contabilizar os tipos de usos mais frequentes
...
# Produtos mais citados 
# Contando a frequência de uso por espécie
df_freq_socio_uso <- tab_socio_ride %>%
  st_drop_geometry() %>% # Apagar Geometria
  group_by(`NOME DESCRITO NO LSA`, PARTE_UTI) %>%
  summarise(Frequencia = n(), .groups = 'drop')

# Criando a formatação desejada
df_formatado_socio_uso <- df_freq_socio_uso %>%
  mutate(`NOME DESCRITO NO LSA` = ifelse(duplicated(`NOME DESCRITO NO LSA`), "", `NOME DESCRITO NO LSA`))

# Exporntando o df para excel
df_formatado_socio_uso %>% export("TABELAS/tabela_sociologica_uso_por_especie.xlsx")

# Exibindo o resultado
# View(df_formatado_socio_uso)

# Identificando os índices onde o valor de NOME DESCRITO NO LSA muda
indices_especies <- which(!duplicated(df_formatado_socio_uso$`NOME DESCRITO NO LSA`))

# A linha horizontal será inserida acima dessas linhas
indices_especies_validos <- indices_especies[indices_especies > 2] 

# Adicionando a linha horizontal acima de cada valor único de NOME DESCRITO NO LSA
flex_formatado_socio_uso <- df_formatado_socio_uso %>%
  st_drop_geometry() %>%
  filter(!`NOME DESCRITO NO LSA` == "Sem informação") %>%
  flextable() %>%
  set_header_labels(
    `NOME DESCRITO NO LSA` = "Nome da Espécie - Popular",
    PARTE_UTI = "Tipos de Usos",
    Frequencia = "Frequência de Respostas"
  ) %>%
  bold(part = "header") %>%
  width(j = 1, width = 1.5) %>%
  width(j = 2, width = 1.5) %>%
  vline(j = c(1:3)) %>%
  # Adicionando a linha horizontal acima de cada valor único de espécie
  hline(i = indices_especies_validos - 1,  # A linha horizontal é colocada acima do índice
...
# Produtos mais citados 
# Contando a frequência de uso por espécie
df_freq_socio_uso_mun <- tab_socio_ride %>%
  st_drop_geometry() %>% # Apagar Geometria
  filter(!`NOME DESCRITO NO LSA` == 'Sem informação') %>% # Filtrar somente os valores únicos
  group_by(`NOME DESCRITO NO LSA`, PARTE_UTI, name_muni) %>%
  summarise(Frequencia = n(), .groups = 'drop') %>%
  select(name_muni, `NOME DESCRITO NO LSA`, PARTE_UTI, Frequencia) %>%
  arrange(name_muni,`NOME DESCRITO NO LSA`)

# Criando a formatação desejada
df_formatado_socio_uso_mun <- df_freq_socio_uso_mun %>%
  mutate(   
         name_muni = ifelse(duplicated(name_muni), "", name_muni))

# Exportar df para excel
df_formatado_socio_uso_mun %>% export("TABELAS/tabela_sociologica_uso_por_municipio.xlsx")

# Identificando os índices onde o valor de NOME DESCRITO NO LSA muda
indices_municipio <- which(!duplicated(df_formatado_socio_uso_mun$name_muni))

# A linha horizontal será inserida acima dessas linhas
indices_municipios_validos <- indices_municipio[indices_municipio > 2]

# Adicionando a linha horizontal acima de cada valor único de NOME DESCRITO NO LSA
flex_formatado_socio_uso_mun <- df_formatado_socio_uso_mun %>%
  st_drop_geometry() %>%
  filter(!`NOME DESCRITO NO LSA` == "Sem informação") %>%
  flextable() %>%
  set_header_labels(
  ...
  vline(j = c(1:4)) %>%
  # Adicionando a linha horizontal acima de cada valor único de espécie
  hline(i = indices_municipios_validos - 1,  # A linha horizontal é colocada acima do índice
        border = fp_border(color = "black", width = 1))
...
```{r}
df_1 <- freq_especie_resposta(tab_socio_ride, "Pequi")

mapa <- mapa_tabela_sociologica(mun_ride, df_1, seq(0, 65, 10))
...
```{r analise-sociologica-especies-mais-usadas-municipio, warning=FALSE, message=FALSE}
